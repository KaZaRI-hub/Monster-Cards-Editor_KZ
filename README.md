...
MonsterCards .card 파일의 텍스트, 이미지, BGM을 수정·저장하고 레이어 미리보기와 후처리를 지원하는 편집 도구입니다.
...
[주요 기능]
- MonsterCards .card 파일을 열어 내부 텍스트, 이미지 bytes, BGM bytes를 확인하고 수정할 수 있습니다.
...
MonsterCards の .card ファイル内のテキスト、画像、BGMを編集・保存し、レイヤープレビューと画像後処理を利用できる編集ツールです。
...
[主な機能]
- MonsterCards の .card ファイルを開き、内部テキスト、画像 bytes、BGM bytes を確認・編集できます。
...
This editor lets you modify and save MonsterCards .card text, images, and BGM, with layer previews and image post-processing tools.
...
[Main Features]
- Open MonsterCards .card files and inspect or edit internal text, image bytes, and BGM bytes.
...
- On save, the .card file is encrypted and compressed again.
...
6. Edit images, descriptions, BGM, and post-processing.
